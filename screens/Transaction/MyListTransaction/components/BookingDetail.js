...
export default function BookingDetail() {
  ...
  const calculateEndTime = () => {
    if (!item.bok_waktu_mulai || !item.bok_durasi_jam) return null;

    const [startHour, startMinute] = item.bok_waktu_mulai
      .split(":")
      .map((v) => parseInt(v));
    if (isNaN(startHour) || isNaN(startMinute)) return null;

    const endHour = (startHour + item.bok_durasi_jam) % 24; // agar tetap dalam format 0–23
    return `${endHour.toString().padStart(2, "0")}:${startMinute.toString().padStart(2, "0")}`;
  };

  console.log("endTime", calculateEndTime());

  const params = {
    book_id: item.bok_id,
    harga_per_jam: item.ruangan.rng_harga_per_jam,
    durasi: item.bok_durasi_jam,
    gambar: item.ruangan.rng_image,
    nama_ruangan: item.ruangan.rng_nama_ruangan,
    totalBooking: item.ruangan.totalBooking,
    date: item.bok_waktu_mulai,
    start: formatDate(item.bok_waktu_mulai, false, true),
    endTime: formatDate(item.bok_waktu_selesai, false, true),
  };
  ...
  const fetchData = () => {
    setLoading(true);
    fetch(`${apiUrl}/TrBooking/` + item.bok_id)
      .then((res) => res.json())
      .then((json) => {
        let initialData = [];
  ...
  return (
    ...
    >
      ...
      />
      <View
        className="pt-12 pb-4 mb-2 px-2"
      ...
      >
        <Text className="text-white text-l font-poppins-bold">
          {item.ruangan.rng_jenis_ruangan}
        </Text>
      </View>
      <View className="bg-[#3F217A] px-4 py-3 rounded-b-2xl">
        <View className="flex-row justify-between items-center">
          <Text className="text-white text-l font-semibold font-poppins-bold">
            {item.ruangan.rng_nama_ruangan}
          </Text>
          <Text className="text-orange-400 text-l font-poppins">
            {formatCurrencyRupiah(item.bok_total_biaya)}
          </Text>
        </View>
      </View>
      ...
      <View className="flex-row">
        ...
        {item.bok_status === "Sesi Selesai" && (
          <TouchableOpacity
            onPress={() => {
              const downloadUrl =
                `${apiUrl}/TrBooking/download-invoice/` + item.bok_id;
              fetch(downloadUrl, {
                method: "GET",
              })
                .then((res) => {
      ...
      {activeTab === "booking" && (
        <>
          <View className="bg-[#3F217A] rounded-xl mt-4 mx-4 p-4">
            <Text className="text-white font-poppins-bold text-base mb-2">
              Play Date & Time
            </Text>
            <View className="flex-row justify-between items-center">
            ...
            <View className="flex-row justify-between">
              <View className="flex-row">
                <Text className="text-white font-poppins text-sm">
                  {formatCurrencyRupiah(item.ruangan.rng_harga_per_jam)}
                </Text>
                <Text className="text-white font-poppins text-sm">
                  {" "}
                  X {item.bok_durasi_jam} Hours
                </Text>
              </View>
              <Text className="text-white font-poppins text-sm">
                {formatCurrencyRupiah(item.bok_total_biaya)}
          ...
          {item.bok_status === "Menunggu Pembayaran" ? (
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate("RequestToPay", { params })}
            >
      ...
      {activeTab === "barcode" && <QRCodeCard qrCode={item.bok_qr_code} />}
    </ImageBackground>
  );
}
