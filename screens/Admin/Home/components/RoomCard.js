...
// Usage:
  ...
  if (item.ruangan.rng_nama_ruangan.includes("VIP")) {
    roomType = "VIP";
  } else if (item.ruangan.rng_nama_ruangan.includes("Private")) {
    roomType = "Private";
  }

  const profileImageUri = item.rng_image
      ? { uri: `${apiUrl}/Images/Ruangan/${item.ruangan.rng_image}` }
          ...
          <View>
            ...
            <View className="flex-row items-center mt-1">
              <Text className="text-xs text-gray-400 ml-1">
                {formatDateToCustomText(item.bok_waktu_mulai.toString())}
              </Text>
            </View>
          </View>
        </View>
